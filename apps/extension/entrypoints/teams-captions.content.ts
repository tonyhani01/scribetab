...
import { findTeamsCaptionsContainer, parseTeamsCaptionNodes } from '@/utils/teamsSelectors';
...
export default defineContentScript({
  ...
  main() {
    ...
    const handleMutations = () => {
      ...
      const now = Date.now();
      const snaps = parseTeamsCaptionNodes(container);
    ...
    const scan = () => {
      if (!capturing) return;
      const found = findTeamsCaptionsContainer(document);

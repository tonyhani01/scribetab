...
import { findZoomCaptionsContainer, parseZoomCaptionNodes } from '@/utils/zoomSelectors';
import type { Ack, ToBackground, ToMeetCaptions } from '@/utils/messages';

const STABILIZE_MS = 400;
const SCAN_MS = 1000;

export default defineContentScript({
  matches: ['https://*.zoom.us/wc/*', 'https://app.zoom.us/wc/*'],
  runAt: 'document_idle',
  main() {
    ...
    const send = (ev: CaptionEvent) => {
      ...
      const message = {
        ...
        endMs: ev.endMs,
      } satisfies Extract<ToBackground, { type: 'CAPTION_EVENT' }>;
      void chrome.runtime
        .sendMessage(message)
        .catch(() => {
    ...
    const handleMutations = () => {
      ...
      const now = Date.now();
      const snaps = parseZoomCaptionNodes(container);
    ...
    const scan = () => {
      if (!capturing) return;
      const found = findZoomCaptionsContainer(document);

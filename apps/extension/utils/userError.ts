...
const RULES: ReadonlyArray<readonly [RegExp, string]> = [
  ...
  [/Stop failed/i, 'Could not stop the recorder cleanly. Audio already captured is saved.'],
  ...
  [/Specified native messaging host|host not installed|not found|native messaging host/i, 'Native host is not installed. Run npx scribetab-host install.'],
  [/forbidden/i, 'Native host access is forbidden. Re-run npx scribetab-host install with this extension ID.'],
...
export function humanError(err: unknown): string {
  ...
  if (raw === GENERIC_USER_ERROR) return raw;
